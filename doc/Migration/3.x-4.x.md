...
Removed `Chubbyphp\Framework\Middleware\RouterMiddleware` (deprecated since 3.x), use `Chubbyphp\Framework\Middleware\RouteMatcherMiddleware`
with a `Chubbyphp\Framework\Router\RouteMatcherInterface` implementation (e.g. `Chubbyphp\Framework\Router\FastRoute\RouteMatcher`).

Removed `Chubbyphp\Framework\Middleware\NewRelicRouteMiddleware`.
...
from `($middleware, $middlewareDispatcher, $emitter)` (3.0)
to `($middleware, $middlewareDispatcher, $requestHandler, $emitter)` (introduced untyped in a 3.x minor, `$requestHandler` is now typed as `?RequestHandlerInterface`)
